...
export type ProTableProps<T, U, ValueType = 'text'> = {
  ...
  ErrorBoundary?: any;
  /**
   * 用于渲染 当Table勾选之后，表头渲染覆盖
   */
  renderSelectHeader?: React.ReactNode | JSX.Element
  /**
   * 用于修正`renderSelectHeader` 布局问题,作用于`renderSelectHeader` 的父容器
   */
  selectHeaderStyle?: Omit<React.CSSProperties, 'margin' | 'marginBlock'>
