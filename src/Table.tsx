...
import { ConfigProvider, Table, Pagination, Select } from 'antd';
...
import { SelectTopRight } from './components/StickPagination'

type tableChangeOptionsProps = {
...
function TableRender<T extends Record<string, any>, U, ValueType>(
  ...
  const [tableChangeOptions, setTableChangeOptions] = useState<tableChangeOptionsProps>({
    changePagination: {},
    filters: {},
    sorter: {},
    extra: { currentDataSource: [], action: 'paginate' }
  })
  /** 需要遍历一下，不然不支持嵌套表格 */
  ...
  const getTableProps = () => ({
    ...
    onChange: (
      ...
      setTableChangeOptions({ changePagination, filters, sorter, extra })
  ...
  const baseTableDom = <Table<T> {...getTableProps()} pagination={false} rowKey={rowKey} />;
  ...
  const replenishDom = useMemo(() => {
    const height = counter.rootDomRef.current?.getElementsByClassName('ant-table-header ant-table-sticky-holder')?.[0]?.clientHeight
    const width = counter.rootDomRef.current?.getElementsByClassName('ant-table-header ant-table-sticky-holder')?.[0]?.clientWidth

    return <div style={{
      position: 'sticky',
      zIndex: 4,
      top: typeof props?.selectHeaderStyle?.marginTop === 'number' ? -(props?.selectHeaderStyle?.marginTop) : - parseFloat(props?.selectHeaderStyle?.marginTop ?? '0'),
      bottom: typeof props?.selectHeaderStyle?.marginBottom === 'number' ? -(props?.selectHeaderStyle?.marginBottom) : - parseFloat(props?.selectHeaderStyle?.marginBottom ?? '0'),
      left: typeof props?.selectHeaderStyle?.marginLeft === 'number' ? -(props?.selectHeaderStyle?.marginLeft) : - parseFloat(props?.selectHeaderStyle?.marginLeft ?? '0'),
      right: typeof props?.selectHeaderStyle?.marginRight === 'number' ? -(props?.selectHeaderStyle?.marginRight) : - parseFloat(props?.selectHeaderStyle?.marginRight ?? '0'),
    }}  >
      {typeof props?.rowSelection ==='object' && props.rowSelection.selectedRowKeys?.length && props?.renderSelectHeader &&
        <div style={{
          height: height,
          ...
          ...props?.selectHeaderStyle
        }} >
          {props.renderSelectHeader}
        </div>
      }
    </div>
  }, [counter.rootDomRef, props?.rowSelection, props.renderSelectHeader])
        ...
        <div style={{
          ...
          <Pagination
            ...
            onChange={(page, pageSize) => props?.onChange?.({
              ...
              tableChangeOptions.filters,
              tableChangeOptions.sorter,
              { currentDataSource: tableChangeOptions.extra.currentDataSource, action: 'paginate' }
            )}
          />
        </div>
      </>
